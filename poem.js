// =========================================================
// 10. poem.js - SINGLE POEM VIEW LOGIC
// =========================================================
...
import { getFirestore, doc, getDoc, updateDoc, increment, collection, query, orderBy, where, addDoc, serverTimestamp, onSnapshot } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
let currentPoemData = null; // Store the current poem data globally

// =========================================================
// 4. HEADER AUTH LOGIC (Copied from main.js for header functionality)
// =========================================================

// (Copy the signInWithGoogle, handleSignOut, onAuthStateChanged logic from main.js here)
// Due to length limitations, ensure you copy the full Auth functions and listeners from main.js!

async function signInWithGoogle() {
    const provider = new GoogleAuthProvider();
    try {
        await signInWithPopup(auth, provider);
        // User details update/save logic is typically done here but is non-critical for this page.
    } catch (error) {
...
newPostLink.addEventListener('click', (e) => { e.preventDefault(); if (auth.currentUser) alert("Post form will open."); else signInWithGoogle(); }); // Simple handling
 ...
 * @returns {string|null} The poem ID or null.
 */
...
async function loadPoem() {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            await trackViewAndUpdateEarnings(docRef);
 ...
 * @param {DocumentReference} poemRef - Reference to the poem document.
 */
async function trackViewAndUpdateEarnings(poemRef) {
    try {
        ...
        // Simple Earning Logic: +1 earning point per 10 views (Future logic can be more complex)
        // Check if the new view count is a multiple of 10
        if ((currentPoemData.views + 1) % 10 === 0) {
             await updateDoc(poemRef, {
                earningsPoints: increment(1)
            });
            // Note: Author's profile earning points update will be managed by a Cloud Function 
            // triggered by the poem document update for accuracy.
        }
        
        // Update UI immediately (non-realtime part)
        viewCountDisplay.textContent = `👁️ ${currentPoemData.views + 1} Views`;
        if ((currentPoemData.views + 1) % 10 === 0) {
             earningPointsDisplay.textContent = `🪙 ${currentPoemData.earningsPoints + 1} Points`;
        }
...
function setupLikeListener(poemRef) {
    // Real-time listener for the overall like count on the poem document
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            if (likeDocSnap.exists()) {
                likeButton.classList.add('liked'); // Add a class for heart color change
                likeButton.querySelector('i').className = 'fas fa-heart'; // Filled heart
            } else {
                likeButton.classList.remove('liked');
                likeButton.querySelector('i').className = 'far fa-heart'; // Empty heart
            }
        } else {
            likeButton.classList.remove('liked');
...
async function handleLikeToggle() {
    ...
    try {
        if (likeButton.classList.contains('liked')) {
            ...
            // Delete the user's like document in the subcollection
            await setDoc(likeDocRef, { active: false }, { merge: true }); // Prefer soft delete/status update
            likeButton.classList.remove('liked');
            likeButton.querySelector('i').className = 'far fa-heart';
        } else {
            // LIKE
        ...
        alert("லைக் செய்வதில் பிழை ஏற்பட்டது.");
    }
}
...
async function handleSubmitComment() {
    ...
    try {
        ...
        await addDoc(collection(db, "poems", currentPoemId, "comments"), {
            ...
            status: 'APPROVED' // Simple initial approval
        });
        ...
        submitCommentBtn.disabled = false;
        setTimeout(() => commentStatus.textContent = '', 5000); // Clear status after 5s
    }
}
...
/**
 * Handles Native Share API usage.
 */
